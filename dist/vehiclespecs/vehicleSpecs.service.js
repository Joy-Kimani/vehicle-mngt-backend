...
export const getAllVehicleSpecsService = async () => {
    const db = getDbPool();
    const result = await db.request().query("SELECT * FROM VehicleSpecs");
    return result.recordset;
};
// get by ID
export const getVehicleSpecsByIdService = async (vehicle_spec_id) => {
    const db = getDbPool();
...
export const createVehicleSpecsService = async (manufacturer, model, year, fuel_type, engine_capacity, transmission, seating_capacity, color, features) => {
    const db = getDbPool();
    const result = await db.request()
...
export const updateVehicleSpecsService = async (vehicle_spec_id, manufacturer, model, year, fuel_type, engine_capacity, transmission, seating_capacity, color, features) => {
    const db = getDbPool();
    const result = await db.request()
        .input("vehicle_spec_id", vehicle_spec_id)
        .input("manufacturer", manufacturer)
...
export const deleteVehicleSpecsService = async (vehicle_spec_id) => {
    const db = getDbPool();
    const result = await db.request()
        .input("vehicle_spec_id", vehicle_spec_id)
