...
export const getActiveBookingsService = async (user_id) => {
...
export const getPendingPaymentsService = async (user_id) => {
    const pool = await getDbPool();
    const query = `
    SELECT 
        p.payment_id,
...
//total rentals done
export const getTotalRentalsDoneService = async (user_id) => {
    const pool = await getDbPool();
    const query = `
...
export const getUpcomingReturnsService = async (user_id, days = 7) => {
    try {
        const pool = await getDbPool();
        const query = `
            SELECT 
...
export const getRecentActivityService = async (user_id) => {
    try {
        const pool = await getDbPool();
