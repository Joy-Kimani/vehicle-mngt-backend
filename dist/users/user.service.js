...
export const updateUserService = async (user_id, first_name, last_name, email, contact_phone) => {
    const db = getDbPool();
...
export const updateUserPasswordService = async (email, hashedPassword) => {
    const db = getDbPool();
    const query = `
    UPDATE Users
...
export const updateUserRoleService = async (user_id, role) => {
...
//     `);
// };
export const toggleUserStatusService = async (user_id) => {
    const db = await getDbPool();
    await db.request()
        .input("user_id", user_id)
        .query(`
