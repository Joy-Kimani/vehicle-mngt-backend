...
/** get by id */
export const getPaymentByIdServices = async (payment_id) => {
    const db = getDbPool();
...
export const createPaymentService = async (booking_id, amount, payment_status, payment_date, payment_method, transaction_id) => {
    const db = getDbPool();
    const result = await db
        .request()
        ...
        .query(`INSERT INTO Payments (booking_id, amount, payment_status, payment_date, payment_method, transaction_id)
       OUTPUT INSERTED.*
       VALUES (@booking_id, @amount, @payment_status, @payment_date, @payment_method, @transaction_id)`);
    return result.recordset[0] || null;
};
/** update */
export const updatePaymentService = async (payment_id, booking_id, amount, payment_status, payment_date, payment_method, transaction_id) => {
    const db = getDbPool();
    const result = await db
        .request()
        .input("payment_id", payment_id)
...
/** delete */
export const deletePaymentService = async (payment_id) => {
    const db = getDbPool();
    const result = await db
        .request()
        .input("payment_id", payment_id)
...
// };
export const initializePaymentServices = async ({ email, amount, booking_id, callback_url, }) => {
    ...
    const res = await axios.post("https://api.paystack.co/transaction/initialize", payload, { headers: { Authorization: `Bearer ${process.env.PAYSTACK_SECRET_KEY}` } });
...
export const getPaymentByIdService = async (payment_id) => {
    const db = getDbPool();
    const result = await db
        .request()
        .input("payment_id", payment_id)
 ...
 */
export const getPaymentsByUserService = async (user_id) => {
    const db = getDbPool();
    const result = await db
        .request()
        .input("user_id", user_id)
...
export const createPaymentRecord = async (booking_id, amount, payment_method, transaction_id) => {
    const db = getDbPool();
    const result = await db
        .request()
        .input("booking_id", booking_id)
        ...
        .input("payment_method", payment_method)
        .input("transaction_id", transaction_id)
        .query(`INSERT INTO Payments (booking_id, amount, payment_status, payment_date, payment_method, transaction_id, created_at)
       OUTPUT INSERTED.*
       VALUES (@booking_id, @amount, 'Pending', GETDATE(), @payment_method, @transaction_id, GETDATE())`);
    return result.recordset[0] || null;
};
...
export const markPaymentSuccessful = async (reference) => {
    const db = getDbPool();
    const result = await db
        .request()
...
export const markBookingPaid = async (booking_id) => {
    const db = getDbPool();
    const result = await db
        .request()
        .input("booking_id", booking_id)
